...
template < int dim >
    struct PoissonProblem {
        ...
        void grid_search(MPI_Comm comm);
...
template <int dim>
void PoissonProblem<dim>::grid_search(MPI_Comm comm) {
    ...
    std::string grid_search_file_name = "data/grid_search_" + std::to_string(rank) + ".csv";
    ...
    for (int i = offset; i < offset + n_rows_l; i++) {
        for (int j = 0; j < n_cols_l; j++) {
            ...
            Teuchos::ParameterList smoothList;
            int chebyshev_degree = (int) (x2 * 10.0 + 1.0);
            smoothList.set("chebyshev: degree", chebyshev_degree);
            paramList.get("smoother: params", smoothList);

            std::cout << "Running precondition/solve with drop tolerance = " << drop_tolerance << ", chebyshev degree = " << chebyshev_degree << std::endl;
